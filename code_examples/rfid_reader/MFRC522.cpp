...
#define RSTPIN RPI_V2_GPIO_P1_22 // 25
...
)
{
	...
	values[index] = bcm2835_spi_transfer(
		0); // Read the final byte. Send 0 to stop reading.
} // End PCD_ReadRegister()
...
)
{
	...
	if (backData && backLen) {
		n = PCD_ReadRegister(
			FIFOLevelReg); // Number of bytes in the FIFO
		if (n > *backLen) {
	...
	if (backData && backLen && checkCRC) {
		...
		n = PCD_CalculateCRC(&backData[0], *backLen - 2,
				     &controlBuffer[0]);
		if (n != STATUS_OK) {
			return n;
		}
...
)
{
	...
	if (*bufferSize != 2 ||
	    validBits != 0) { // ATQA must be exactly 16 bits.
		return STATUS_ERROR;
	}
	return STATUS_OK;
...
)
{
	...
	signed char
		currentLevelKnownBits; // The number of known UID bits in the current Cascade Level.
	...
	while (!uidComplete) {
		...
		while (!selectDone) {
			...
			if (currentLevelKnownBits >=
				...
				buffer[6] = buffer[2] ^ buffer[3] ^ buffer[4] ^
					    buffer[5];
				// Calculate CRC_A
				result =
				...
				index = 2 +
					count; // Number of whole bytes: SEL + NVB + UIDs
				buffer[1] =
					(index << 4) +
					txLastBits; // NVB - Number of Valid Bits
			...
			if (result ==
				...
				byte collisionPos =
					result &
					0x1F; // Values 0-31, 0 means bit 32.
				if (collisionPos == 0) {
					collisionPos = 32;
				}
				if (collisionPos <=
				...
				count = (currentLevelKnownBits - 1) %
					8; // The bit to modify
				index = 1 + (currentLevelKnownBits / 8) +
					(count ? 1 :
						 0); // First byte is index 0.
				buffer[index] |= (1 << count);
			} else if (result != STATUS_OK) {
				return result;
			} else { // STATUS_OK
				if (currentLevelKnownBits >=
				    32) { // This was a SELECT.
					selectDone =
						true; // No more anticollision
		...
		index = (buffer[2] == PICC_CMD_CT) ?
				3 :
				2; // source index in buffer[]
		...
		if (responseLength != 3 ||
		    txLastBits !=
			    0) { // SAK must be exactly 24 bits (1 byte + CRC_A).
			return STATUS_ERROR;
		}
...
)
{
	...
	for (char blockOffset = no_of_blocks - 1; blockOffset >= 0;
	     blockOffset--) {
		...
		if (no_of_blocks == 4) {
			...
			group = blockOffset / 5;
			firstInGroup = (group == 3) ||
				       (group != (blockOffset + 1) / 5);
		}
...
bool MFRC522::MIFARE_OpenUidBackdoor(bool logErrors)
{
	...
	cmd = 0x43;
	validBits = 8;
	status = PCD_TransceiveData(&cmd, (byte)1, response, &received,
				    &validBits, (byte)0, false); // 43
	if (status != STATUS_OK) {
		if (logErrors) {
...
bool MFRC522::MIFARE_SetUid(byte *newUid, byte uidSize, bool logErrors)
{
	...
	byte status = PCD_Authenticate(MFRC522::PICC_CMD_MF_AUTH_KEY_A, (byte)1,
				       &key, &uid);
	if (status != STATUS_OK) {
		if (status == STATUS_TIMEOUT) {
			...
			status = PCD_Authenticate(
				MFRC522::PICC_CMD_MF_AUTH_KEY_A, (byte)1, &key,
				&uid);
			if (status != STATUS_OK) {
