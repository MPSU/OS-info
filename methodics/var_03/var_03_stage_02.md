...
__Задание:__  
1. Скачайте на ПК исполняемые файлы программ с репозитория курса [color_sense](../../code_examples/Color%20Sensor%20Module/color_sense) и [led_blinker](../../code_examples/LED%20Blinker/led_blinker).
...
4. Запустите с различными аргументами через консоль putty поочередно программы `color_sense` и `led_blinker` и убедитесь, что программы выдают код распознаваемого цвета `code: xxx` и цвет горящего светодиода, переключаемого по таймауту.
5. Запустите программы `color_sense` и `led_blinker` в фоновом режиме, при этом перенаправив их вывод для записи в один файл.
...
1. Как работают программы `color_sense` и `led_blinker`, какие аргументы передаются в них при запуске и на что они влияют?
...
4. Оцените сколько процессорного времени занимают программы `color_sense` и `led_blinker` как при одновременной работе, так и при работе по отдельности?
5. Что произойдет, если запустить по несколько программ `color_sense` и `led_blinker` одновременно?
6. Каким образом можно перенаправить вывод программ в файл? Что происходит при одновременном обращении к одному файлу?
7. Каким условиям не удовлетворяют программы `color_sense` и `led_blinker` для выполнения Вашего курсового проекта?
8. Что необходимо поменять в этих программах, чтобы они удовлетворяли требованиям, предъявляемым к программам -1,-2?
9. Изучите исходные коды программ `color_sense` и `led_blinker`, оцените, соответствуют ли они правилам написания исходных кодов ПО?
