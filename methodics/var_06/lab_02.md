...
__Задание:__  
1. Скачайте на ПК исполняемые файлы программ с репозитория курса [encoder](../../code_examples/Rotary%20Encoder/encoder) и [keypad](../../code_examples/Клавиатура/keypad).
...
4. Запустите с различными аргументами через консоль putty поочередно программы `encoder` и `keypad ` и убедитесь, что программы при вращении энкодера выводят значение приращения `angle increment: ` и при нажатии на клавиатуру выводят в консоль нажимаемый символ.
5. Запустите программы `encoder` и `keypad ` в фоновом режиме, при этом перенаправив их вывод для записи в один файл.
...
1. Как работают программы `encoder` и `keypad `, какие аргументы передаются в них при запуске и на что они влияют?
...
4. Оцените сколько процессорного времени занимают программы `encoder` и `keypad ` как при одновременной работе, так и при работе по отдельности?
5. Что произойдет, если запустить по несколько программ `encoder` и `keypad ` одновременно?
6. Каким образом можно перенаправить вывод программ в файл? Что происходит при одновременном обращении к одному файлу?
7. Каким условиям не удовлетворяют программы `encoder` и `keypad ` для выполнения Вашего курсового проекта?
8. Что необходимо поменять в этих программах, чтобы они удовлетворяли требованиям, предъявляемым к программам -1,-2?
9. Изучите исходные коды программ `encoder` и `keypad `, оцените, соответствуют ли они правилам написания исходных кодов ПО?
