...
__Задание:__  
1. Скачайте на ПК исполняемые файлы программ с репозитория курса [nav_module](../../code_examples/nav_module) и [rangefinder_hcsr04](../../code_examples/rangefinder_hcsr04).
...
4. Запустите с различными аргументами через консоль putty поочередно программы `nav_data` и `rangefinder_hcsr04 ` и убедитесь, что программы выдают в консоль данные навигационного модуля (углы ориентации, ускорения по осям) и различные отсчеты АЦП при изменении расстояния до преграды.
5. Запустите программы `nav_data` и `rangefinder_hcsr04 ` в фоновом режиме, при этом перенаправив их вывод для записи в один файл.
...
1. Как работают программы `nav_data` и `rangefinder_hcsr04`, какие аргументы передаются в них при запуске и на что они влияют?
...
4. Оцените сколько процессорного времени занимают программы `nav_data` и `rangefinder_hcsr04` как при одновременной работе, так и при работе по отдельности?
5. Что произойдет, если запустить по несколько программ `nav_data` и `rangefinder_hcsr04` одновременно?
6. Каким образом можно перенаправить вывод программ в файл? Что происходит при одновременном обращении к одному файлу?
7. Каким условиям не удовлетворяют программы `nav_data` и `rangefinder_hcsr04` для выполнения Вашего курсового проекта?
8. Что необходимо поменять в этих программах, чтобы они удовлетворяли требованиям, предъявляемым к программам -1,-2?
9. Изучите исходные коды программ `nav_data` и `rangefinder_hcsr04`, оцените, соответствуют ли они правилам написания исходных кодов ПО?
