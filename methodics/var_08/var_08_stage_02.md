...
__Задание:__  
1. Скачайте на ПК исполняемые файлы программ с репозитория курса [rfid_reader](../../code_examples/Считыватель%20RFID-RC522/rfid_reader) и [display_text](../../code_examples/LCD%20display/display_text).
...
4. Запустите с различными аргументами через консоль putty поочередно программы `rfid_reader` и `display_text ` и убедитесь, что программы выдают в консоль сообщение об успешном считывании RF-метки `read ok!` и выводят текст из консоли на дисплей.
5. Запустите программы `rfid_reader` и `display_text ` в фоновом режиме, при этом перенаправив их вывод для записи в один файл.
...
1. Как работают программы `rfid_reader` и `display_text`, какие аргументы передаются в них при запуске и на что они влияют?
...
4. Оцените сколько процессорного времени занимают программы `rfid_reader` и `display_text` как при одновременной работе, так и при работе по отдельности?
5. Что произойдет, если запустить по несколько программ `rfid_reader` и `display_text` одновременно?
6. Каким образом можно перенаправить вывод программ в файл? Что происходит при одновременном обращении к одному файлу?
7. Каким условиям не удовлетворяют программы `rfid_reader` и `display_text` для выполнения Вашего курсового проекта?
8. Что необходимо поменять в этих программах, чтобы они удовлетворяли требованиям, предъявляемым к программам -1,-2?
9. Изучите исходные коды программ `rfid_reader` и `display_text`, оцените, соответствуют ли они правилам написания исходных кодов ПО?
