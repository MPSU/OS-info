...
__Задание:__  
1. Скачайте с репозитория исходные коды программ `display_text_ФИО` и `encoder_ФИО`, make файлы и bash-скрипт для их запуска, скопируйте их в соответсвующие поддиректории папки `./stage_05`.
2. Создайте поддиректорию `combiner` для третьей программы.
3. Напишите исходные коды программы `combiner`, которая бы последовательно обменивалась сообщениями по именованным каналам от программ `display_text_ФИО` и `encoder_ФИО`, а также обрабатывала бы нажатия на кнопки BUTTON0/BUTTON1/SW (кнопка энкодера) для управления выводом: копка SW энкодера - вход/выход из режима настройки времени, BUTTON0 - переключение изменяемого поля времени справа налево, BUTTON1 - слева направо. Вывод программы `combiner` должен удовлетворять требованиям задания и иметь следующие типы сообщений:
* «№х. Сигнал на изменение времени ______ (hh:mm:ss)»;
* «№х. Время изменено на ______ (hh:mm:ss)».
...
3. Может ли программа `combiner` считывать значения с 3-х кнопок одновременно?
4. Каким образом реализован прием сообщений от программ `display_text_ФИО` и `encoder_ФИО` в программе `combiner`, может ли быть пропущенно сообщение от какой-либо программы?
